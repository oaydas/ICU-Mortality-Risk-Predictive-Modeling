...
def load_features(split: str, n_jobs: int = -1) -> tuple[npt.NDArray, pd.DataFrame, list[str]]:
    """Use functions to load and preprocess the feature vectors for a given split.
...
def get_project_data(
    debug: bool = False
) -> tuple[npt.NDArray, npt.NDArray, npt.NDArray, npt.NDArray, list[str], list[str]]:
...
def get_challenge_data() -> tuple[npt.NDArray, npt.NDArray, npt.NDArray, list[str]]:
    ...
    X_challenge, _, held_features = load_features("challenge")
    return X_train, y_train, X_challenge, feature_names, held_features
